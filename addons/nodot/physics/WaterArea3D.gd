...
var water_material = load("res://addons/nodot/shaders/clean_water.tres")
...
func _ready():
  ...
  var waterMeshInstance = MeshInstance3D.new()
  ...
  waterMesh.material = water_material
  waterMeshInstance.mesh = waterMesh
  waterMeshInstance.transparency = 0.05
  waterMeshInstance.cast_shadow = false
...
func _physics_process(delta: float):
  ...
  for idx in body_tracker.size():
    ...
    for p in probes:
      var depth = get_wave_height(p) - p.y 
      if depth > 0:
