...
func _init():
  if enabled:
    var action_names = [left_action, right_action, up_action, down_action, jump_action, "escape"]
    var default_keys = [JOY_AXIS_LEFT_X, JOY_AXIS_LEFT_X, JOY_AXIS_LEFT_Y, JOY_AXIS_LEFT_Y, JOY_BUTTON_A, JOY_BUTTON_START]
...
func _physics_process(delta: float) -> void:
  if enabled and !is_editor:
    ...
    var direction = camera_container.global_transform.basis * Vector3(input_direction_x, 0, input_direction_y)
    if direction:
