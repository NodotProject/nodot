...
## Dispatches a round. If the weapon requires charging, it starts the charging process.
## If the weapon is reloading, it returns immediately.
## If there are enough rounds left in the chamber, it discharges a round.
...
func reload() -> void:
	...
	if supply_count != -1 and (rounds_required <= 0 or supply_count < rounds_required): return
	if supply_count != -1:
...
func resupply(amount: int) -> int:
	...
	if new_supply > supply_count_limit:
		var rejected = new_supply - supply_count_limit
