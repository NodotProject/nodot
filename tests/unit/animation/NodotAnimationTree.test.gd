...
func test_request_timeseek():
	anim_tree.request_timeseek("TimeSeek", 1.5)
	expect_equal(anim_tree.get_param("TimeSeek/seek_request"), 1.5, "Should request time seek")

func test_set_and_get_timescale():
	anim_tree.set_timescale("TimeScale", 2.0)
	expect_equal(anim_tree.get_timescale("TimeScale"), 2.0, "Should set and get time scale")

func test_set_and_get_blend_amount():
	anim_tree.set_blend_amount("Blend2", 0.7)
	expect_equal(anim_tree.get_blend_amount("Blend2"), 0.7, "Should set and get blend amount for Blend2/3")
...
func test_lerp_blend_amount():
	anim_tree.set_blend_amount("Blend2", 0.0)
	anim_tree.lerp_blend_amount("Blend2", 1.0, 0.5)
	expect_true(is_equal_approx(anim_tree.get_blend_amount("Blend2"), 0.5), "Should lerp blend amount")
...
func test_request_transition():
	anim_tree.request_transition("Transition", "my_transition")
	expect_equal(anim_tree.get_param("Transition/transition_request"), "my_transition", "Should request a transition")

func test_state_machine_travel():
	var playback = anim_tree.get_state_machine_playback("StateMachine")
	expect_true(playback is AnimationNodeStateMachinePlayback)
